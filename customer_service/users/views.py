...
def __create_account(username, password, email):
    ...
    new_account = Account(id=id, username=username, password=password, email=email)
    new_account.save()
...
def __create_fullname(first_name, last_name):
    id = random.randint(100000000, 99999999999999)
    new_fullname = FullName(id = id, first_name=first_name, last_name=last_name)
    new_fullname.save()
...
def __create_address(address, city, country):
    id = random.randint(100000000, 99999999999999)
    new_address = Address(id = id, address=address, city=city, country=country)
    new_address.save()
...
def __create_customer(account_id, full_name_id, address_id):
    id = random.randint(100000000, 99999999999999)
    new_customer = Customer(id=id, account_id=account_id, full_name_id=full_name_id, address_id=address_id)
    new_customer.save()
    ...
    if request.method == 'POST':
        if 'application/json' in request.META['CONTENT_TYPE']:
            ...
            if account_id and full_name_id and address_id:
                respdata = __create_customer(account_id, full_name_id, address_id)
                resp['status'] = 'Success'
                resp['status_code'] = '200'
