...
def __create_author(name, email, address):
    ...
    new_author = Author(id=id, name=name, email=email, address=address)
    new_author.save()
...
def __create_book(title, author_id, category_id, published_date, price, description):
    id = random.randint(100000000, 99999999999999)
    new_book = Book(id = id, title=title, author_id=author_id, category_id=category_id, published_date=published_date, price=price, description=description)
    new_book.save()
    return new_book.id
    ...
    if request.method == 'POST':
        if 'application/json' in request.META['CONTENT_TYPE']:
            ...
            if title and author_id and category_id and published_date and price and description:
                respdata = __create_book(title, author_id, category_id, published_date, price, description)
                resp['status'] = 'Success'
                resp['status_code'] = '200'
...
def author_search(request):
    ...
    if request.method == 'POST':
        if 'application/json' in request.META['CONTENT_TYPE']:
            ...
            if name:
                authors = Author.objects.filter(name__icontains=name).values('id', 'name', 'email', 'address')
                if authors:
                    resp['status'] = 'Success'
                    resp['status_code'] = '200'
...
def category_search(request):
    ...
    if request.method == 'POST':
        if 'application/json' in request.META['CONTENT_TYPE']:
            ...
            if name:
                categories = Category.objects.filter(name__icontains=name).values('id', 'name')
                if categories:
                    resp['status'] = 'Success'
                    resp['status_code'] = '200'
