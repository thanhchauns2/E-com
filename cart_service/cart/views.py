...
def add_item_to_cart(request):
    if request.method == 'POST':
        if 'application/json' in request.META['CONTENT_TYPE']:
            ...
            response_data = {'cart_id': cart.cart_id, 'user_id': cart.user_id, 'items': cart.items}
...
def clear_cart(request):
    if request.method == 'POST':
        if 'application/json' in request.META['CONTENT_TYPE']:
            ...
            # Trả về thông tin của cart đã được cập nhật
            response_data = {'cart_id': cart.cart_id, 'user_id': cart.user_id, 'items': cart.items, 'message' : 'Cleared cart successfully.'}
            return HttpResponse(json.dumps(response_data), content_type='application/json')
